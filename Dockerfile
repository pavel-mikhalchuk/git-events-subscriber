...
RUN go build ./cmd/git-events-subscriber

# Let's create a /dist folder containing just the files necessary for runtime.
# Later, it will be copied as the / (root) of the output image.
WORKDIR /dist
RUN cp /build/git-events-subscriber ./git-events-subscriber
...
RUN ldd git-events-subscriber | tr -s '[:blank:]' '\n' | grep '^/' | \
...
ENTRYPOINT ["/git-events-subscriber"]